...
| APPROVED | cancel | CANCELLED | not purchase_locked |

- 그 외 모든 `(from, action)` → `AppError("INVALID_TRANSITION", 409)`.
- `REJECTED`, `CANCELLED` 은 종료 상태 (out-edge 없음).
- 재제출은 상태 전이가 아님 → 신규 `POST /quotes` (원본 `REJECTED` 유지).

## 가드 (`can_mutate(quote) -> None | raise`)

수정(PUT) / 삭제(DELETE) / cancel 공통 선행 검사:

1. `quote.deleted_at` → `NOT_FOUND`
2. `quote.purchase_locked` → `PURCHASE_LOCKED` (409)
3. 수정/삭제 시 `quote.status in (REJECTED, CANCELLED)` → `INVALID_TRANSITION` (읽기전용 보존)
