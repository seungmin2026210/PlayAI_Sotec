...
| PUT | `/quotes/{id}` | 전체관리자 | 수정 (전체 필드). 잠금/반려됨/취소됨이면 409. 상태 유지 |
| DELETE | `/quotes/{id}` | 전체관리자 | soft delete + 결번. 잠금이면 409 |
| POST | `/quotes/{id}/approve` | 전체관리자 | SUBMITTED→APPROVED. 그 외 상태 409 |
| POST | `/quotes/{id}/reject` | 전체관리자 | body `{reason}` 필수. SUBMITTED→REJECTED. reason 없으면 422 |
| POST | `/quotes/{id}/cancel` | 전체관리자 | SUBMITTED/APPROVED→CANCELLED. 잠금이면 409 |
...
| 메서드 | 경로 | 권한 | 설명 |
|---|---|---|---|
| GET | `/quotes/{id}/export.xlsx` | 인증(조회권 있는 자) | 개별 엑셀 |
| GET | `/quotes/{id}/export.pdf` | 인증 | 개별 PDF |
| GET | `/quotes/export.xlsx` | 인증 | 목록 엑셀 (동일 필터 파라미터 적용, 그룹관리자 스코프 강제) |
