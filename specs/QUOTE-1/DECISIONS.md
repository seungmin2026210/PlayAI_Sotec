...
- 개별 견적서 로고를 뺐다 — 원본 위탁계약용 양식에 로고 영역이 없음(상단은 "견적NO" 라벨 차지).
  목록 엑셀은 로고 그대로 유지.
- PDF(`build_quote_pdf`)는 **이번 범위 밖** — 엑셀이 확정된 뒤 별도 작업으로 재작성한다.
  문구 상수는 위탁계약형 값을 그대로 물려받지만(레이아웃 깨짐 없이 계속 동작), 섹션 구성(8열
  항목표·서명란 등)은 옛 SW형 그대로라 엑셀과 셀 단위는 물론 섹션 구성까지 다르다 — 의도된
  임시 상태.
